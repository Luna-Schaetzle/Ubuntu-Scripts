...
# Funktion, um den Stundenplan für den aktuellen Tag abzurufen
def fetch_timetable():
    try:
        ...
        today = datetime.datetime.now().date()  # Aktuelles Datum
        table = session.my_timetable(start=today, end=today)  # Nur für heute
        ...
        if formatted_timetable:
            ...
            text_area.insert(tk.END, "Keine Einträge gefunden für heute.")
...
fetch_button = tk.Button(root, text="Stundenplan für heute abrufen", command=fetch_timetable, bg="#4CAF50", fg="white", font=("Helvetica", 12))
